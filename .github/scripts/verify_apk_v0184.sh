...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='118400' versionName='0.18.4'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.18.4 Native Jinhak Bridge + Adiga Audit'" >/dev/null
...
echo "v0.18.4 APK verified: metadata + stable signer $SIGNER"
