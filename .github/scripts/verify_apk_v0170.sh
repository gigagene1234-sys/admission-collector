...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='117000' versionName='0.17.0'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.17.0 Server-Owned Auth'" >/dev/null
...
echo "v0.17.0 APK verified: version, label, stable signer $SIGNER"
