...
echo "$DUMP" | grep -F "versionCode='116600'" >/dev/null
echo "$DUMP" | grep -F "versionName='0.16.6'" >/dev/null
echo "$DUMP" | grep -F "application-label:'Admission Hub v0.16.6 High3 Route Isolation'" >/dev/null
...
echo "v0.16.6 APK verified: version, label, stable signer"
