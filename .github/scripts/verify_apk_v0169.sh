...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='116900' versionName='0.16.9'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.16.9 Event-Driven High3 Auth'" >/dev/null
...
echo "v0.16.9 APK verified: version, label, stable signer $SIGNER"
