...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='117100' versionName='0.17.1'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.17.1 User-Owned Jinhak Session'" >/dev/null
...
echo "v0.17.1 APK verified: metadata + stable signer $SIGNER"
