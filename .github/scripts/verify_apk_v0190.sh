...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='119000' versionName='0.19.0'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.19 Storage Competition Monitor'" >/dev/null
...
echo "v0.19.0 APK verified: metadata + stable signer $SIGNER"
