...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='118200' versionName='0.18.2'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.18.2 Protected Session Bootstrap'" >/dev/null
...
echo "v0.18.2 APK verified: metadata + stable signer $SIGNER"
