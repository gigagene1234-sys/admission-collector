...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='118100' versionName='0.18.1'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.18.1 Focused Six Missions'" >/dev/null
...
echo "v0.18.1 APK verified: metadata + stable signer $SIGNER"