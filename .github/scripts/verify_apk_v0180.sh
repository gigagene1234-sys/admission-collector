...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='118000' versionName='0.18.0'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.18.0 Dedicated Auto Login'" >/dev/null
...
echo "v0.18.0 APK verified: metadata + stable signer $SIGNER"
