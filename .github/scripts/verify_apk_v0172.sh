...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='117200' versionName='0.17.2'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.17.2 Bootstrap-Safe Jinhak Session'" >/dev/null
...
echo "v0.17.2 APK verified: metadata + stable signer $SIGNER"
