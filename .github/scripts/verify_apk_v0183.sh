...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='118300' versionName='0.18.3'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.18.3 Storage Competition Watch'" >/dev/null
...
echo "v0.18.3 APK verified: metadata + stable signer $SIGNER"
