...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='117400' versionName='0.17.4'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.17.4 Strict High3 Sandbox'" >/dev/null
...
echo "v0.17.4 APK verified: metadata + stable signer $SIGNER"
