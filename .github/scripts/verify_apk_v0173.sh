...
echo "$BADGING" | grep -F "package: name='com.admissionhub.collector' versionCode='117300' versionName='0.17.3'" >/dev/null
echo "$BADGING" | grep -F "application-label:'Admission Hub v0.17.3 Natural High3 Handoff'" >/dev/null
...
echo "v0.17.3 APK verified: metadata + stable signer $SIGNER"
