...
object OfficialUniversityScoreCalculator {
    ...
    fun calculate(candidate: JSONObject, profile: JSONObject): JSONObject {
        ...
        if (!profile.optBoolean("completeTranscriptConfirmedByUser", false)) return hold(base, "transcript-not-confirmed-complete", "가져온 과목·학기가 판단에 필요한 학생부 전체를 포함하는지 확인이 필요합니다.")
