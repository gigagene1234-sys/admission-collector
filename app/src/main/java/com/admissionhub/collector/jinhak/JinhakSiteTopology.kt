...
enum class JinhakMissionLane(val wireName: String, val basePriority: Int) {
    SAVED_APPLICATIONS("saved-applications", 100), CURRENT_PREDICTION("current-prediction", 96), MOCK_SUPPORT("mock-support", 94),
    ACTUAL_ADMIT("actual-admit", 98), UNIVERSITY_RESULT("university-result", 92), SCORE_ANALYSIS("score-analysis", 88),
    RECOMMENDATION("recommendation", 48), STRATEGY("strategy", 82), ADMISSION_KNOWLEDGE("admission-knowledge", 76),
    REFERENCE("reference", 35), MEDIA("media", 12), UNKNOWN("unknown", 5)
}
...
object JinhakSiteTopology {
    ...
    private const val LEGACY_HIGH3_PROTECTED_SCORE = "https://hijinhak.jinhak.com/Nesin/MyGrade/UserScoreNew.aspx"

    /**
     * Protected high3 route that has produced stable authenticated proof on the actual device.
     * Authentication is proved here before the collector enters the newer www.jinhak.com mission UI.
     */
    fun authProofRoute(): String = LEGACY_HIGH3_PROTECTED_SCORE

    fun authProofRoutes(): List<String> = listOf(LEGACY_HIGH3_PROTECTED_SCORE, missionSeeds().first())
    ...
    fun lane(url: String, label: String = ""): JinhakMissionLane {
        val lower = url.lowercase(); val path = runCatching { URI(url).path?.lowercase().orEmpty() }.getOrDefault(lower); val text = "$lower ${label.lowercase()}"
        return when {
            path.contains("/nesin/mygrade/userscorenew.aspx") -> JinhakMissionLane.SCORE_ANALYSIS
    ...
    fun priority(url: String, label: String = ""): Int {
        val lane = lane(url, label); var score = lane.basePriority; val text = "$url $label"
    ...
    fun isDefaultSusiCoreTraversalUrl(url: String, label: String = ""): Boolean = when (lane(url, label)) {
        JinhakMissionLane.SAVED_APPLICATIONS, JinhakMissionLane.CURRENT_PREDICTION, JinhakMissionLane.MOCK_SUPPORT,
        JinhakMissionLane.ACTUAL_ADMIT, JinhakMissionLane.UNIVERSITY_RESULT, JinhakMissionLane.SCORE_ANALYSIS,
        JinhakMissionLane.STRATEGY, JinhakMissionLane.ADMISSION_KNOWLEDGE -> true
        JinhakMissionLane.REFERENCE, JinhakMissionLane.RECOMMENDATION, JinhakMissionLane.MEDIA, JinhakMissionLane.UNKNOWN -> false
    }
    fun shouldExpandEditorial(url: String, label: String = ""): Boolean = when (lane(url, label)) {
        JinhakMissionLane.UNIVERSITY_RESULT, JinhakMissionLane.STRATEGY, JinhakMissionLane.ADMISSION_KNOWLEDGE -> true
        else -> false
    }
}
