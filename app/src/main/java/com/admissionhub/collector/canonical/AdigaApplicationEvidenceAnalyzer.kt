 ...
 * Separately, the UI can state when the current admission track and recruitment unit are each
 * officially verified. Count fields remain authoritative even when the bounded evidence sample
 * does not happen to contain a direct historical row.
 */
object AdigaApplicationEvidenceAnalyzer {
    ...
    fun analyze(candidate: JSONObject): JSONObject {
        ...
        for (i in 0 until evidence.length()) {
            ...
            when {
                ...
                directlyBound && recordYear in 2000 until academicYear -> {
                    ...
                    row.optJSONObject("historicalOutcome")?.let {
                        compact.put("historicalOutcome", JSONObject(it.toString()))
                        historicalOutcomes.put(JSONObject(it.toString()).put("sourcePage", row.optString("sourcePage")))
                    }
        ...
        val knownCurrentDirectCount = maxOf(
            currentBound.length(),
            binding.optInt("officialStructuralCurrent", 0),
            binding.optInt("officialRowBoundCurrent", 0) + binding.optInt("officialTableSegmentCurrent", 0)
        )
        val knownHistoricalDirectCount = maxOf(
            historicalBound.length(),
            binding.optInt("officialTableSegmentHistorical", 0)
        )
        val boundedSampleMissingKnownDirectEvidence =
            knownCurrentDirectCount > currentBound.length() || knownHistoricalDirectCount > historicalBound.length()
        ...
        } else facts.put("지원년도 전형·모집단위 직접 결합 근거 ${knownCurrentDirectCount}건이 확인되었습니다.")

        if (knownHistoricalDirectCount == 0) missing.put("과거 입결에서 전형과 모집단위를 동시에 묶는 공식 행")
        else facts.put("과거 동일 전형·모집단위 직접 연결 근거 ${knownHistoricalDirectCount}건이 확인되었습니다.")
        if (historicalOutcomes.length() > 0) facts.put("표 머리글까지 검증해 읽어낸 공식 과거 입결 행 ${historicalOutcomes.length()}건이 있습니다.")
        if (boundedSampleMissingKnownDirectEvidence) facts.put("카드의 근거 예시는 일부만 보관되므로 직접 근거 원문은 전체 로컬 어디가 자료 재검색에서 확인합니다.")

        val code = when {
            knownCurrentDirectCount > 0 && knownHistoricalDirectCount > 0 -> "BOUND_CURRENT_AND_HISTORICAL"
            knownCurrentDirectCount > 0 -> "BOUND_CURRENT_ONLY"
            currentComponentsVerified && knownHistoricalDirectCount > 0 -> "CURRENT_COMPONENTS_AND_HISTORICAL_BOUND"
        ...
        val label = when (code) {
            ...
            "CURRENT_COMPONENTS_AND_HISTORICAL_BOUND" -> "어디가: 2027 전형·모집단위 각각 공식 확인 + 과거 동일 조합 입결 직접 확인"
        ...
        val nextAction = when (code) {
            ...
            "CURRENT_COMPONENTS_AND_HISTORICAL_BOUND" -> "지원년도에는 전형표와 모집단위표가 분리되어 있어 임의로 한 행으로 합치지 않습니다. 전형과 모집단위는 각각 공식 확인되었고, 과거 동일 조합 입결 행은 별도로 직접 확인되었습니다."
        ...
        return JSONObject()
            ...
            .put("currentApplicationBoundCount", knownCurrentDirectCount)
            .put("historicalApplicationBoundCount", knownHistoricalDirectCount)
            .put("historicalOutcomeCount", historicalOutcomes.length())
            .put("boundedEvidenceSampleTruncated", boundedSampleMissingKnownDirectEvidence)
