...
class MainActivity : Activity() {
    ...
    private lateinit var jinhakSessionConfirmButton: Button
    ...
    private var jinhakV0912AuthCompatibilityTransitions = 0
    ...
    private var jinhakV0172BootstrapStarts = 0
    ...
    private var jinhakV0173ResumeArmed = false
    ...
    private var jinhakV0180AuthState = "IDLE"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        scoreReviewUi = ScoreReviewUi(this, localStore, { canonicalHubSessionId() }, { refreshHubDashboardFromStore("score-review") }, {
            startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply {
                addCategory(Intent.CATEGORY_OPENABLE)
                type = "*/*"
                putExtra(Intent.EXTRA_MIME_TYPES, arrayOf("text/csv", "text/tab-separated-values", "text/plain", "application/json"))
            }, IMPORT_SCORE_REQUEST)
        })
        buildUi()
    ...
    private fun buildUi() {
        ...
        root.addView(LinearLayout(this).apply {
            ...
            addView(Button(this@MainActivity).apply { text = "6장 원서 적정성 검토"; setOnClickListener { scoreReviewUi.showPortfolio() } }, LinearLayout.LayoutParams(0, -2, 1f))
        })
    ...
    private fun renderHubDashboard(model: JSONObject) {
        ...
        if (::hubDecisionSummary.isInitialized) {
            ...
            val portfolio = model.optJSONObject("applicationPortfolio")?.optJSONObject("counts") ?: JSONObject()
            val own = if (profile.has("ownWeightedGrade") && !profile.isNull("ownWeightedGrade")) " · 입력 내신 %.3f".format(java.util.Locale.US, profile.optDouble("ownWeightedGrade")) else ""
            hubDecisionSummary.text = "$profileText ${profile.optInt("rowCount", 0)}과목$own\n공식 검증 환산 ${summary.optInt("verifiedConversions", 0)}/6 · 공식 입결 ${summary.optInt("officialOutcomeAvailable", 0)}/6 · 진학사 수치 ${summary.optInt("structuredPredictions", 0)}/6\n원서 검토 가능 ${portfolio.optInt("REVIEWABLE")} · 성적 위험 ${portfolio.optInt("SCORE_RISK")} · 조건 재검토 ${portfolio.optInt("CONDITIONS_RECHECK")} · 자료 보완 ${portfolio.optInt("HOLD")}"
    ...
    private fun showHubDashboardCard(slot: Int) {
        ...
        AlertDialog.Builder(this)
            .setTitle("지원 $slot 상세")
            .setMessage(message)
            .setNeutralButton("원서 적정성·근거") { _, _ -> scoreReviewUi.showCard(card.optString("applicationIdentityKey")) }
            .setPositiveButton("확인", null)
            .show()
    }
    ...
    private fun startSelectedSixRecovery() {
        ...
        currentBatchTarget = null
        return
        val coreProbe = JinhakSiteTopology.protectedCoreProbeUrl()
    ...
    private fun jinhakAuthCompatibilityWindowActive(): Boolean = provider == ProviderId.JINHAK && (
    ...
    private fun openProvider(which: ProviderId) {
        if (startupLoginPreflightActive) {
            Toast.makeText(this, "로그인 상태 확인 중에는 사이트 전환을 로그인 오케스트레이터가 관리합니다.", Toast.LENGTH_SHORT).show()
            return
        }
        if (unifiedRunning) {
    ...
    private fun refreshSessionOrOpenLogin() {
        ...
        checkSessionState { needsLogin, authenticated ->
            ...
            status.text = if (needsLogin) {
                "로그인이 필요한 상태가 감지되었습니다. 현재 화면을 유지하며 로그인 폼이 나타나면 직접 로그인하세요."
            } else {
        ...
        webView.evaluateJavascript(js) { encoded ->
            try {
                ...
                sessionState.text = when {
                    ...
                    loginSurface -> "○ 로그인 화면 감지 · 로그인 상태 확인"
    ...
    private fun missingJinhakCoreLanes(identityKey: String): Set<String> {
    ...
    private fun nextJinhakIncompleteCoverageRecoveryCandidate(
        ...
        for (identity in incomplete) {
            ...
            // The candidate comes directly from the persistent SAME-APPLICATION ledger, so it
            // bypasses the ordinary live-candidate seen-key filter without mutating confirmed state.
    ...
    private fun persistJinhakMissionRuntimeState(trigger: String, mutatedTarget: JSONObject? = null) {
        ...
        localStore.storeJinhakMissionRuntime(
            ...
            JSONObject()
                ...
                .put("targetAuthRedirectEpisodeOpenKey", JSONObject.NULL)
    ...
    private fun providerLoginUrl(which: ProviderId): String = when (which) {
        ProviderId.JINHAK -> JinhakStrictHigh3Sandbox.sanitizedHigh3OrNull(currentBatchTarget) ?: JinhakStrictHigh3Sandbox.strictEntryUrl()
        ...
        delays.forEach { delay ->
            handler.postDelayed({
                ...
                probeLoginSurface(which) { probe ->
                    ...
                    status.text = "${which.displayName} 로그인 폼을 실제 DOM에서 감지했습니다. 현재 공식 사이트 화면에서 직접 로그인하세요."
    ...
    private fun isFreshJinhakRealAuthProbe(): Boolean = false

    private fun classifyJinhakRealAuthRoute(rawUrl: String): String {
        val canonical = canonicalizeBatchUrl(rawUrl)
        val core = canonicalizeBatchUrl(JinhakSiteTopology.protectedCoreProbeUrl())
        return when {
    ...
    private fun startAutomaticLoginAndCollectionSequence(trigger: String) {
        ...
        if (unifiedRunning || batchRunning) {
            Toast.makeText(this, "진행 중인 수집이 있어 로그인 상태 확인를 시작할 수 없습니다.", Toast.LENGTH_LONG).show()
            return
        }
        ...
        if (startupLoginAdigaRestoredLease && startupLoginJinhakRestoredLease) {
            ...
            recordRuntimeEvent("startup-stored-session-verification-required", JSONObject()
                ...
                .put("credentialExported", false)
                .put("sessionSecretStoredLocally", false)
                .put("sessionSecretExported", false))
        }

        startupLoginPollGeneration += 1
        unifiedButton.text = "로그인 확인 취소"
    ...
    private fun beginStartupLoginProvider(which: ProviderId) {
        ...
        localRunId = localStore.latestResumableRun(which.wireName)
        CookieManager.getInstance().flush()
        val lease = if (which == ProviderId.JINHAK) null else runCatching { sessionVault.restore(which.wireName) }.getOrNull()
    ...
    private fun evaluateStartupLoginStateLegacyAdiga(expectedProvider: ProviderId, generation: Int) {
            ...
            probeLoginSurface(expectedProvider) { probe ->
                ...
                if (probe.optBoolean("detected", false)) {
                    ...
                    if (credentialVault.has(expectedProvider.wireName)) {
                        status.text = "${expectedProvider.displayName} 로그인 화면이 감지되었습니다. 직접 로그인하세요."
                        ...
                        status.text = "${expectedProvider.displayName} 로그인 화면이 감지되었습니다. 공식 사이트 화면에서 직접 로그인하세요."
                ...
                if (startupAuthIndeterminatePolls >= 2) {
                    ...
                    if (isProviderLoginUrl(expectedProvider, currentUrl)) {
                        ...
                        if (credentialVault.has(expectedProvider.wireName)) {
                            ...
                            status.text = "로그인이 필요합니다. 공식 사이트 화면에서 직접 로그인하세요."
                            showCredentialDialog(expectedProvider, continueAfterSave = true)
                        }
                        scheduleStartupLoginPoll(expectedProvider, generation)
                        return@probeLoginSurface
                ...
                status.text = "${expectedProvider.displayName} 현재 화면을 유지합니다. 로그인이 필요한 경우 직접 로그인하도록 안내합니다."
                scheduleStartupLoginPoll(expectedProvider, generation)
            }
        }
    }
    ...
    private fun onStartupProviderAuthenticated(expectedProvider: ProviderId, generation: Int) {
        ...
        if (expectedProvider == ProviderId.ADIGA) {
            ...
            if (jinhakLastCoreVerifiedAtMs == 0L && canonicalizeBatchUrl(webView.url.orEmpty()) == canonicalizeBatchUrl(JinhakSiteTopology.protectedCoreProbeUrl())) {
                jinhakLastCoreVerifiedAtMs = System.currentTimeMillis()
                jinhakLastAuthEvidence = "protected-core-stable"
            }
        ...
        recordRuntimeEvent("startup-login-provider-authenticated", JSONObject()
            .put("provider", expectedProvider.wireName)
            .put("credentialStoredLocally", credentialVault.has(expectedProvider.wireName)).put("credentialExported", false).put("sessionSecretStoredLocally", false).put("sessionSecretExported", false))
        if (expectedProvider == ProviderId.ADIGA) {
    ...
    private fun startUnifiedCollectionAuthenticated() {
        ...
        jinhakV0912AuthCompatibilityTransitions = 0
        ...
        localStore.recordSyncState(
            ...
            JSONObject()
                ...
                .put("bothProviderLeasesRestoreAttemptedAtBootstrap", true)
                ...
                .put("loginPreflight", JSONObject()
                    ...
                    .put("passwordStored", credentialVault.has(ProviderId.ADIGA.wireName) || credentialVault.has(ProviderId.JINHAK.wireName))
                    .put("sessionSecretStoredLocally", false)
                    .put("sessionSecretExported", false)
                ...
                .put("hardLeaseRecoveryDispatches", jinhakHardLeaseRecoveryDispatches)
                    ...
                    .put("v0172BootstrapStarts", jinhakV0172BootstrapStarts)
                    ...
                    .put("v0173ResumeArmed", jinhakV0173ResumeArmed)
    ...
    private fun hasFreshProtectedCoreProofForTargetRedirect(): Boolean = false
    ...
    private fun isJinhakV0173StaleLoginCallback(callbackUrl: String, source: String): Boolean {
        ...
        handler.postDelayed({
            ...
            if (accepted) {
                ...
                handler.postDelayed({
                    ...
                    val retryOrigin = currentBatchTarget
                    if (!retryOrigin.isNullOrBlank() && isProviderUrl(retryOrigin)) loadJinhakV0174High3Only(retryOrigin, "remaining-retry-origin")
                    else loadNextBatchPage()
                }, 350L)
                return@postDelayed
            } else {
        ...
        if (unifiedRunning && unifiedPhase == "adiga" && provider == ProviderId.ADIGA) {
            ...
            if (sessionId != null) {
                ...
                localStore.recordSyncState(
                    ...
                    .put("jinhakV0912AuthCompatibilityTransitions", jinhakV0912AuthCompatibilityTransitions)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == IMPORT_SCORE_REQUEST && resultCode == RESULT_OK) {
            ...
            Thread {
                val result = runCatching {
                    contentResolver.openInputStream(uri)?.bufferedReader(Charsets.UTF_8)?.use { reader ->
                        ...
                        while (true) { val n = reader.read(buffer); if (n < 0) break; text.append(buffer, 0, n); require(text.length <= 1_000_000) { "성적 파일은 1MB 이하여야 합니다." } }
                        text.toString()
                    } ?: error("파일을 열 수 없습니다.")
                }
                runOnUiThread { result.onSuccess { scoreReviewUi.showProfile(it) }.onFailure { Toast.makeText(this, it.message ?: "UTF-8 성적 파일을 확인하세요.", Toast.LENGTH_LONG).show() } }
            }.start()
            return
        }
            ...
            Thread {
                val result = runCatching {
                    contentResolver.openOutputStream(uri)?.bufferedWriter()?.use { writer ->
                        if (streamSession != null) localStore.writeUnifiedExport(streamSession, writer, VERSION, BUILD_CODE)
