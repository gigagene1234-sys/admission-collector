...
class JinhakAuthEventStateTest {
    ...
    @Test
    fun genericProductLoginIsPassiveSiteOwnedWait() {
        val url = "https://www.jinhak.com/jh/member/login"
        assertEquals(JinhakAuthEventState.Event.MEMBER_LOGIN, JinhakAuthEventState.classify(url))
        assertEquals(JinhakAuthEventState.Action.WAIT_FOR_SERVER_RETURN, JinhakAuthEventState.action(url))
    }

    @Test
    fun memberLoginWaitsForServerReturnWithoutCollectorRewrite() {
    ...
    @Test
    fun high3NavigationEventIsHandedToCollectorWithoutLoginRewrite() {
    ...
    @Test
    fun unknownHigh3PageIsStillHigh3AndNotStopped() {
